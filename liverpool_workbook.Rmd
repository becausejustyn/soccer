...
```{r}
xg_data <- get_match_results(
  country = "ENG", 
  gender = "M", 
  season_end_year = c(2017:2022)
  )

saveRDS(xg_data, "xg_data.rds")

xg_data <- read_rds("data/xg_data.rds")
```
...
```{r}
end_season_summary1 <- end_season_summary %>%
  mutate(
    goals_above_expected = GF - xG)
```

```{r}
end_season_summary1 %>%
  #filter(Season_End_Year == 2022) %>%
  select(Squad, goals_above_expected, Season_End_Year) %>%
  arrange(-goals_above_expected)
```
...
end_season_summary %>% 
  ...
  ggplot(aes(
    x = goals_above_expected, 
    y = Pts)) +
  geom_point(colour = "black", fill = "orange", shape = 21, size = 3) +
  labs(
    x = "Goals above expected", 
    y = "Season Points",
    caption = "\n*Data current to 10/01/2022\nSource: worldfootballR",
    title = "EXCEEDING EXPECTED GOALS AND MORE POINTS",
    subtitle = "For the 2017/18 to 2020/21 seasons, teams tended to earn more 
...
liv_cum_totals <- xg_data %>% 
  select(
    Season_End_Year, Date, Team = Home, 
  ...
  bind_rows(
    xg_data %>%
      select(
        Season_End_Year, Date, Team = Away, 
        Goals = AwayGoals, xG = Away_xG) %>%
      mutate(
        HomeAway = "Away", goals_above_expected = Goals - xG
        )
    ) %>%
  ...
  select(
    Season_End_Year, game_num, 
    Goals = cum_total_goals, xG = cum_total_xg) %>% 
...
liv_cum_totals %>%
  ...
  geom_text(data = lab_df, 
    aes(
      ...
      label = GoalType, 
      colour = GoalType), 
    vjust = 0, family = "chivo") +
  scale_colour_manual(
    values = c("#C31014", alpha("#C31014", 0.4))) +
