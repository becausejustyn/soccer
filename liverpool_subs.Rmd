...
`https://www.dontblamethedata.com/blog/liverpools-early-substitutions-worldfootballr/`

`https://github.com/JaseZiv/DontBlameTheData/blob/master/content/post/reds_subs_analysis.Rmd`
...
write_rds(match_summaries, "match_summaries.rds")
```
...
`https://fivethirtyeight.com/features/bundesliga-teams-could-be-using-more-substitutions-but-they-arent/`

```{r}
subs_data <- match_summaries %>%
  filter(str_detect(tolower(event_type), "substitut")) %>%
  mutate(
    in_first_half = ifelse(event_time <= 45, "yes", "no"),
...
sub_analysis <- subs_data %>%
  ...
  group_by(Game_URL) %>%
  mutate(earliest_sub_game = min(event_time, na.rm = T)) %>%
  ungroup() %>%
  group_by(Season, Team, restart_state) %>%
  mutate(
    GP = n_distinct(Game_URL),
    n_sub = n(),
  ...
  mutate(
    share_subs_45 = sub_first_half / n_sub,
...
subs_data %>%
  ...
  labs(
    title = "TIME OF FIRST SUB BACK TO PRE-COVID LEVELS",
    subtitle = "Teams were making their first sub a lot earlier after the restart in the 19/20 season,\nbut have since reverted back to pre-restart times*",
    caption = "*Data current to 11/01/2021\nSource: worldfootballR"
    ) +
  theme_minimal() +
  theme(
    axis.title.y = element_blank(), axis.title.x = element_text(hjust = 1, family = "chivo", size = 14, colour = "grey20"),
...
liverpool_seasons <- subs_data %>%
  ...
  mutate(
    share_subs_45_reds = sub_first_half_reds / n_sub_reds,
...
all_other_seasons <- sub_analysis %>%
  ...
  group_by(Season, restart_state) %>%
  summarise(
    avg_team_subs_season = round(mean(n_sub, na.rm = T), 4),
...
final_table <- combined %>%
  mutate(
    avg_team_subs_season = round(avg_team_subs_season),
  ...
  tab_spanner(
    ...
    columns = c(n_sub_reds, avg_team_subs_season)
  ) %>%
  tab_spanner(
    label = "Avg Time to 1st Sub",
    columns = c(avg_earliest_sub_reds, avg_earliest_sub_teams)
  ) %>%
  tab_spanner(
    label = "Subs/Game",
    columns = c(avg_subs_game_reds, avg_subs_game_team)
  ) %>%
  tab_style(
    style = list(
    ...
    locations = cells_body(
      columns = c(share_subs_60_reds, avg_share_subs_60),
      rows = "2020-2021"
    )
  ) %>%
  tab_spanner(
