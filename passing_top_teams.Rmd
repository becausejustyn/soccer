...
theme_set(theme_minimal() +
  theme(
    plot.title = element_text(colour = "black", size = 22, family = "chivo", face = "bold"),
...
write_rds(league_table, "league_table.rds")
...
season_league_names <- league_table %>%
  select(Season_End_Year, Competition_Name) %>%
  arrange(desc(Season_End_Year), Competition_Name) %>%
  distinct(Competition_Name, .keep_all = TRUE)
```
...
p1_data <- passing %>%
  ...
  mutate(is_focus_league = ifelse(Competition_Name == "La Liga", "La Liga", ifelse(Competition_Name == "Ligue 1", "Ligue 1", "Other"))) %>%
  ungroup()
```
...
p1_data %>%
  ggplot(aes(
    x = Season_End_Year, 
    y = passess_attempted_90_league, 
    group = Competition_Name, 
    colour = is_focus_league
    )) +
  geom_line(size = 1) +
  ...
  labs(
    x = "Season End",
    y = "Passes per 90 mins",
    title = "NOT AS MUCH PASSING IN LA LIGA GAMES",
    ...
    caption = paste0("\n*Data current to ", lubridate::today(), "\nSource: worldfootballR/fbref.com")
  ) +
...
passing %>%
  ...
  ggplot(aes(
    ...
    y = Att_per_90, 
    colour = Competition_Name, 
    fill = Competition_Name
    )) +
  geom_boxplot(alpha = 0.5) +
  labs(
    x = "Season End", y = "Passes Attempted",
    title = "NOT ALL TEAMS IN THE LEAGUE ARE CREATED EQUAL",
...
passing %>%
  ...
  labs(
    x = "Season End",
    y = "Completion %",
    title = "ALL LEAGUES COMPLETING MORE PASSES",
...
passing %>%
  ...
  labs(
    x = "Passes Attempted", 
    y = "League Points",
    title = "THE BETTER TEAMS ALSO ATTEMPT MORE PASSES",
...
directness_df <- passing %>%
  ...
  group_by(Competition_Name, Country, Season_End_Year) %>%
  summarise(
    total_dist = sum(TotDist_Total),
    total_prg_dist = sum(PrgDist_Total)
  ) %>%
  mutate(directness = total_prg_dist / total_dist) %>%
  ungroup()
```
...
directness_df %>%
  ...
  labs(
    x = "Season End",
    y = "Directness (Yards)",
    title = "PASSING BECOMING LESS DIRECT IN ALL LEAGUES",
...
directness_cor <- passing %>%
  ...
  filter(Season_End_Year != 2021) %>%
  group_by(Competition_Name) %>%
  summarise(cors = cor(team_directness, Pts)) %>%
  ungroup()
```
...
passing %>%
  ...
  scale_colour_manual(values = league_cols) +
  labs(
    x = "Directness", y = "League Points",
    title = "BETTER TO NOT BE AS DIRECT?",
...
passing %>%
  ...
  tab_source_note(
    source_note = md("SOURCE: [worldfootballR](https://github.com/JaseZiv/worldfootballR)")
  ) %>%
  cols_label(
  ...
  tab_header(
    title = md("**PREMIER LEAGUE DOMINATES LEAST DIRECT PASSING**"),
...
passing %>%
  ...
  tab_header(
    title = md("**MORE DIRECT PASSING TEAMS GENERALLY WEAKER TEAMS?**"),
